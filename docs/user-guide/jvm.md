...
#### 使用 2MB 巨页代替默认的 4KB 小页（Linux 巨页）（慎用）

效果：减少 TLB 未命中，提升 GC 吞吐量 10%\~15%，降低 CPU 开销。
代价：需要操作系统预先分配巨页池，配置稍复杂
目前已知的问题：如果操作系统的内存池中没有符合要求的干净的内存区域，会导致 JVM 启动失败。
解决方法：

1. 确保操作系统的内存池中有足够的干净内存区域。
2. 如果是虚拟机，尝试调整虚拟机的内存分配策略，如 -XX:UseLargePages=310 可以帮助。
3. 如果是物理机，检查是否有足够的物理内存。
4. 改用透明大页（Transparent Huge Pages）（THP）。
...
1. Xmx 必须是 2 的整数倍比如1,2，4，6，8，16，32，64
2. Xms 必须是 2 的整数倍比如1,2，4，6，8，16，32，64
